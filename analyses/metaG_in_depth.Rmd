...
# Code dependencies:
require("vegan"); require("reshape"); require("ggplot2"): require("tidyr"); require("dplyr"); require("dbstats"); require("reshape2"); require("ggpubr"); require("glue"); require("tidyverse"); require("gasfluxes")
```
...
```{r Import metadata and soil data}
#Design file:
...
```{r read in meta data}
#Read in design file
meta <- read.csv("../data/tmg_design.csv")
...
select_fluxes_combined_w_T0 <- ch4_fluxes_only_w_T0[,c(1,5)] # Start with CH4 IDs and f0 for a base dataframe.

colnames(select_fluxes_combined_w_T0)[2] <- "f0_CH4" # rename to distinguish between GHGs

select_fluxes_combined_w_T0$f0_CO2 <- co2_fluxes_only_w_T0$f0_selected # add on CO2 fluxes

select_fluxes_combined_w_T0$f0_N2O <- n2o_fluxes_only_w_T0$f0_selected # add on N2O fluxes
...
# For compatibility and to make a new DF for changes:
GHG_less_columns <- select_fluxes_combined_w_T0
...
# Add on box:
GHG_less_columns$box <- ghg_ppm_filtered$chamber

#Subset GHG_less_columns for 'start' time point (6/13/2016) and 'finish' time point (8/11/2016):
GHG_start <- dplyr::filter(GHG_less_columns, date_short == "6/13")
GHG_finish <- dplyr::filter(GHG_less_columns, date_short == "8/11")

#Add design2 into GHG 'start' and 'finish':
GHG_start <- left_join(GHG_start, design2, by = c("plant", "treatment", "history", "box"))
GHG_finish <- left_join(GHG_finish, design2, by = c("plant", "treatment", "history", "box"))

#Subset GHG_finish and GHG_start to only have metaG sample numbers:
GHG_start_metaG <- rbind(GHG_start[5:6,], GHG_start[11:12,], GHG_start[15:24,], GHG_start[31:32,])
GHG_finish_metaG <- rbind(GHG_finish[5:6,], GHG_finish[11:12,], GHG_finish[15:24,], GHG_finish[31:32,])

#To align "pre" metaG samples with a single, initial GHG value (from the start GHG measurements on 6/28/2016), average together GHG samples in the same box number, then transpose and set row name to corresponding metaG Sample ID:
GHG_start_03 <- data.frame(colMeans(GHG_start_metaG[1:2, 2:4]))
GHG_start_03 <- as.data.frame(t(GHG_start_03))
row.names(GHG_start_03) <- "03Pre"

GHG_start_06 <- data.frame(colMeans(GHG_start_metaG[3:4, 2:4]))
GHG_start_06 <- as.data.frame(t(GHG_start_06))
row.names(GHG_start_06) <- "06Pre"

GHG_start_08 <- data.frame(colMeans(GHG_start_metaG[5:6, 2:4]))
GHG_start_08 <- as.data.frame(t(GHG_start_08))
row.names(GHG_start_08) <- "08Pre"

GHG_start_09 <- data.frame(colMeans(GHG_start_metaG[7:8, 2:4]))
GHG_start_09 <- as.data.frame(t(GHG_start_09))
row.names(GHG_start_09) <- "09Pre"

GHG_start_10 <- data.frame(colMeans(GHG_start_metaG[9:10, 2:4]))
GHG_start_10 <- as.data.frame(t(GHG_start_10))
row.names(GHG_start_10) <- "10Pre"

GHG_start_11 <- data.frame(colMeans(GHG_start_metaG[11:12, 2:4]))
GHG_start_11 <- as.data.frame(t(GHG_start_11))
row.names(GHG_start_11) <- "11Pre"

GHG_start_12 <- data.frame(colMeans(GHG_start_metaG[13:14, 2:4]))
GHG_start_12 <- as.data.frame(t(GHG_start_12))
row.names(GHG_start_12) <- "12Pre"

GHG_start_16 <- data.frame(colMeans(GHG_start_metaG[15:16, 2:4]))
...
GHG_finish_metaG <- as.data.frame(GHG_finish_metaG) # must be a DF for row.names to work
...
#Combine GHG_start_metaG_avg and GHG_finish_metaG[,2:4] to have all 24 samples:
GHG_metaG <- rbind(GHG_start_metaG_avg, GHG_finish_metaG[,2:4])
...
GHG_finish_envfit <- GHG_finish_metaG %>%
  dplyr::select(-c(date_short, time, source, ttreat, hhistory, ID))
```
...
colnames(GHG_finish_envfit)[7] <- "chamber"
...
# To prep merge, make dataframe and set rownames to match
SFO.envfit <- as.data.frame(SFO.envfit)
rownames(SFO.envfit) <- rownames(GHG_finish_envfit)

# Remove GHG_finish_envfit chamber and rename $Sample to chamber
GHG_finish_envfit <- GHG_finish_envfit[,-7]
colnames(GHG_finish_envfit)[7] <- "chamber"

# Merge soil and greenhouse gas data:
envfit_FINISH <- merge(GHG_finish_envfit, SFO.envfit, by= c("plant", "treatment", "history", "chamber"))
...
start.Assim_SO4_red.dbplsr <- dbplsr(GHG_start_metaG_avg$f0_CO2 ~ as.matrix(Assim_SO4_redBC_start), ncomp = 7, method = "GCV")
summary(start.Assim_SO4_red.dbplsr)
#plot(start.Assim_SO4_red.dbplsr)

finish.Assim_SO4_red.dbplsr <- dbplsr(GHG_finish_metaG$f0_CO2 ~ as.matrix(Assim_SO4_redBC_finish), ncomp = 15, method = "GCV")
summary(finish.Assim_SO4_red.dbplsr)
#plot(finish.Assim_SO4_red.dbplsr)

# CH4 ~ Dissimilatory sulfate reduction:
start.Dissim_SO4_red.dbplsr <- dbplsr(GHG_start_metaG_avg$f0_CH4~ as.matrix(Dissim_SO4_redBC_start), ncomp = 7, method = "GCV")
summary(start.Dissim_SO4_red.dbplsr)
#plot(start.Dissim_SO4_red.dbplsr)

finish.Dissim_SO4_red.dbplsr <- dbplsr(GHG_finish_metaG$f0_CH4 ~ as.matrix(Dissim_SO4_redBC_finish), ncomp = 15, method = "GCV")
summary(finish.Dissim_SO4_red.dbplsr)
#plot(finish.Dissim_SO4_red.dbplsr)

# CH4 ~ Fe reduction:
start.FeReduction.dbplsr <- dbplsr(GHG_start_metaG_avg$f0_CH4 ~ as.matrix(FeReductionBC_start), ncomp = 7, method = "GCV")
summary(start.FeReduction.dbplsr)
#plot(start.FeReduction.dbplsr)

finish.FeReduction.dbplsr <- dbplsr(GHG_finish_metaG$f0_CH4 ~ as.matrix(FeReductionBC_finish), ncomp = 15, method = "GCV")
summary(finish.FeReduction.dbplsr)
#plot(finish.FeReduction.dbplsr)

# CH4 ~ methanogenesis:
start.meth.dbplsr <- dbplsr(GHG_start_metaG_avg$f0_CH4 ~ as.matrix(methBC_start), ncomp = 7, method = "GCV")
summary(start.meth.dbplsr)
#plot(start.meth.dbplsr)

finish.meth.dbplsr <- dbplsr(GHG_finish_metaG$f0_CH4 ~ as.matrix(methBC_finish), ncomp = 15, method = "GCV")
summary(finish.meth.dbplsr)
#plot(finish.meth.dbplsr)

# CH4 ~ methane oxidation:
start.CH4_ox.dbplsr <- dbplsr(GHG_start_metaG_avg$f0_CH4 ~ as.matrix(CH4_oxBC_start), ncomp = 7, method = "GCV")
summary(start.CH4_ox.dbplsr)
#plot(start.CH4_ox.dbplsr)

finish.CH4_ox.dbplsr <- dbplsr(GHG_finish_metaG$f0_CH4 ~ as.matrix(CH4_oxBC_finish), ncomp = 15, method = "GCV")
summary(finish.CH4_ox.dbplsr)
#plot(finish.CH4_ox.dbplsr)

# CO2 ~ central carbohydrate metabolism:
start.carb_metab.dbplsr <- dbplsr(GHG_start_metaG_avg$f0_CO2 ~ as.matrix(carb_metabBC_start), ncomp = 7, method = "GCV")
summary(start.carb_metab.dbplsr)
#plot(start.carb_metab.dbplsr)

finish.carb_metab.dbplsr <- dbplsr(GHG_finish_metaG$f0_CO2 ~ as.matrix(carb_metabBC_finish), ncomp = 15, method = "GCV")
summary(finish.carb_metab.dbplsr)
#plot(finish.carb_metab.dbplsr)

# CO2 ~ Cytochrome C Oxidase:
start.cytoCox.dbplsr <- dbplsr(GHG_start_metaG_avg$f0_CO2 ~ as.matrix(cytoCoxBC_start), ncomp = 7, method = "GCV")
summary(start.cytoCox.dbplsr)
#plot(start.cytoCox.dbplsr)

finish.cytoCox.dbplsr <- dbplsr(GHG_finish_metaG$f0_CO2 ~ as.matrix(cytoCoxBC_finish), ncomp = 15, method = "GCV")
summary(finish.cytoCox.dbplsr)
#plot(finish.cytoCox.dbplsr)

# CO2 ~ Denitrification:
start.denit.dbplsr.co2 <- dbplsr(GHG_start_metaG_avg$f0_CO2 ~ as.matrix(denitBC_start), ncomp = 7, method = "GCV")
summary(start.denit.dbplsr.co2)
#plot(start.denit.dbplsr.co2)

finish.denit.dbplsr.co2 <- dbplsr(GHG_finish_metaG$f0_CO2 ~ as.matrix(denitBC_finish), ncomp = 15, method = "GCV")
summary(finish.denit.dbplsr.co2)
#plot(finish.denit.dbplsr.co2)

# N2O ~ Denitrification:
start.denit.dbplsr.n2o <- dbplsr(GHG_start_metaG_avg$f0_N2O ~ as.matrix(denitBC_start), ncomp = 7, method = "GCV")
summary(start.denit.dbplsr.n2o)
#plot(start.denit.dbplsr.n2o)

finish.denit.dbplsr.n2o <- dbplsr(GHG_finish_metaG$f0_N2O ~ as.matrix(denitBC_finish), ncomp = 15, method = "GCV")
summary(finish.denit.dbplsr.n2o)
#plot(finish.denit.dbplsr.n2o)
```
...
methane.dbplsr.plot <- ggplot(data = dbplsr_comp2, aes(x=Timepoint, y=Comp2_adjR2, fill = Dist_matrix)) +
  geom_bar(stat = "identity", position = position_dodge())+
  scale_fill_manual(values=c("#0072B2","#D55E00","#009E73"), name = "KEGG Module")+
  theme_bw() +
...
#ggsave("../figures/Fe_S_manuscript/methane_dbplsr_plot.png", plot=methane.dbplsr.plot, device="png", path=NULL, scale=1, width=7, height=4, dpi=600, limitsize=TRUE, bg = "white")
```
...
adonis_denit_H = adonis2(denitRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = denitRA_design, perm=10000, set.seed=42, by = "terms")
adonis_denit_H

adonis_meth_H = adonis2(methRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = methRA_design, perm=10000, set.seed=42, by = "terms")
adonis_meth_H

adonis_carb_metab_H = adonis2(carb_metabRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = carb_metabRA_design, perm=10000, set.seed=42, by = "terms")
adonis_carb_metab_H

adonis_cytoCox_H = adonis2(cytoCoxRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = cytoCoxRA_design, perm=10000, set.seed=42, by = "terms")
adonis_cytoCox_H

adonis_SO4_S_Assim_H = adonis2(SO4_S_AssimRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = SO4_S_AssimRA_design, perm=10000, set.seed=42, by = "terms")
adonis_SO4_S_Assim_H

adonis_Assim_SO4_red_H = adonis2(Assim_SO4_redRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = Assim_SO4_redRA_design, perm=10000, set.seed=42, by = "terms")
adonis_Assim_SO4_red_H

adonis_Dissim_SO4_red_H = adonis2(Dissim_SO4_redRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = Dissim_SO4_redRA_design, perm=10000, set.seed=42, by = "terms")
adonis_Dissim_SO4_red_H

adonis_ThioSOX_H = adonis2(ThioSOXRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = ThioSOXRA_design, perm=10000, set.seed=42, by = "terms")
adonis_ThioSOX_H

adonis_DNRA_H = adonis2(DNRARA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = DNRARA_design, perm=10000, set.seed=42, by = "terms")
adonis_DNRA_H

adonis_ANRA_H = adonis2(ANRARA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = ANRARA_design, perm=10000, set.seed=42, by = "terms")
adonis_ANRA_H

adonis_Nitrific_H = adonis2(NitrificRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = NitrificRA_design, perm=10000, set.seed=42, by = "terms")
adonis_Nitrific_H

adonis_COMAMMOX_H = adonis2(COMAMMOXRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = COMAMMOXRA_design, perm=10000, set.seed=42, by = "terms")
adonis_COMAMMOX_H

adonis_Nfix_H = adonis2(NfixRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = NfixRA_design, perm=10000, set.seed=42, by = "terms")
adonis_Nfix_H

adonis_CH4_ox_H = adonis2(CH4_oxRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = CH4_oxRA_design, perm=10000, set.seed=42, by = "terms")
adonis_CH4_ox_H

adonis_mtrBC_TIGR_H = adonis2(mtrBC_TIGRRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = mtrBC_TIGRRA_design, perm=10000, set.seed=42, by = "terms")
adonis_mtrBC_TIGR_H

adonis_FeGenie_H = adonis2(FeGenieRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = FeGenieRA_design, perm=10000, set.seed=42, by = "terms")
adonis_FeGenie_H

adonis_SO4_red_combined_H = adonis2(SO4_red_combinedRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = SO4_red_combinedRA_design, perm=10000, set.seed=42, by = "terms")
adonis_SO4_red_combined_H

adonis_FeReduction_H = adonis2(FeReductionRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = FeReductionRA_design, perm=10000, set.seed=42, by = "terms")
adonis_FeReduction_H

adonis_FeOxidation_H = adonis2(FeOxidationRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = FeOxidationRA_design, perm=10000, set.seed=42, by = "terms")
...
vec_meth_SSF <- subset(vec_meth_SSF, A_meth_SSF$pvals<=0.1) # alpha = 0.1 for EcoSphere paper
...
plot2b <- ggplot(df2b, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_segment(data=vec_meth_SSF, aes(x=0,xend=V1,y=0,yend=V2), linewidth=1, arrow = arrow(length = unit(0.2, "cm")),colour="black", alpha=0.4, inherit.aes=F)+ 
 geom_text(data=vec_meth_SSF, aes(x=V1, y=V2-0.005), label= "%moisture", size=4, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
vec_carb_metab_SSF <- subset(vec_carb_metab_SSF, A_carb_metab_SSF$pvals<=0.1) # alpha = 0.1 for EcoSphere paper
...
plot2c <- ggplot(df2c, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
 ...
 geom_text(data=vec_carb_metab_SSF, aes(x=V1-0.01, y=V2-0.005), label= "%paint removed (redox)", size=4, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
explainvar1_carb_metab_FIN # 36.9
explainvar2_carb_metab_FIN # 19.4
...
vec_cytoCox_SSF <- subset(vec_cytoCox_SSF, A_cytoCox_SSF$pvals<=0.1) # alpha = 0.1 for EcoSphere paper
...
explainvar1_cytoCox_FIN # 41.4
explainvar2_cytoCox_FIN # 18.4
...
#ggsave("../figures/Ecosphere_pub/Figure5.tiff", plot = MetaG_pcoa_4panel_SSF, device="tiff", path=NULL, scale=1, height = 7, width = 10, limitsize = T, bg = "white", dpi = 600)

#ggsave("../figures/Ecosphere_pub/metaG_pcoa4panel.jpg", plot = MetaG_pcoa_4panel_SSF, device=NULL, path=NULL, scale=1, height = 7, width = 10, limitsize = TRUE, dpi = 600)
...
# add vector labels for envfit on PCoA:
vec_SO4_S_Assim_SSF$vec_labels <- "%paint removed (redox)"
...
vec_Assim_SO4_red_SSF$X_nudge <- 0
vec_Assim_SO4_red_SSF$Y_nudge <- 0.005

# Add column with vector labels for envfit on PCoA:
vec_Assim_SO4_red_SSF$vec_labels <- "%paint removed (redox)"
...
# Add a column with nudges for nudge_x and nudge_y:
vec_Assim_SO4_red_SFO$X_nudge <- c(0, -0.006, -0.005, -0.005, -0.01, -0.01, -0.005, -0.01, -0.01, 0)
vec_Assim_SO4_red_SFO$Y_nudge <- c(0.007, 0, 0.005, 0.005, 0, 0, 0.005, 0, 0, 0.005)
...
plot5b <- ggplot(df5b, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_text(data=vec_Assim_SO4_red_SFO, aes(x=V1, y=V2), label= vec_Assim_SO4_red_SFO$labels, size=4, nudge_x = vec_Assim_SO4_red_SFO$X_nudge, nudge_y = vec_Assim_SO4_red_SFO$Y_nudge, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
# Add column with labels, for plotting:
vec_Dissim_SO4_red_SSF$labels <- expression(paste("CH"[4]))
...
plot4c <- ggplot(df4c, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_segment(data=vec_Dissim_SO4_red_SSF, aes(x=0,xend=V1,y=0,yend=V2), linewidth=1, arrow = arrow(length = unit(0.2, "cm")),colour="black", alpha = 0.4,inherit.aes=F)+ 
  geom_text(data=vec_Dissim_SO4_red_SSF, aes(x=V1+0.005, y=V2-0.005), label=vec_Dissim_SO4_red_SSF$labels, size=4, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
# Add a column with nudges for nudge_x and nudge_y:
vec_Dissim_SO4_red_SFO$X_nudge <- c(-0.01, 0, 0.01, 0)
vec_Dissim_SO4_red_SFO$Y_nudge <- c(0, 0.005, 0, 0.005)
...
plot5c <- ggplot(df5c, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_text(data=vec_Dissim_SO4_red_SFO, aes(x=V1, y=V2), label= vec_Dissim_SO4_red_SFO$labels, size=4, nudge_x = vec_Dissim_SO4_red_SFO$X_nudge, nudge_y = vec_Dissim_SO4_red_SFO$Y_nudge, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
#ggsave("../figures/Fe_S_manuscript/S_pcoa4panel_SSF.png", plot = S_pcoa_4panel_SSF, device=NULL, path=NULL, scale=1, height = 7, width = 12, limitsize = TRUE, dpi = 600)

#ggsave("../figures/Fe_S_manuscript/Figure3.tiff", plot = S_pcoa_4panel_SSF, device="tiff", path=NULL, scale=1, height = 7, width = 13, limitsize = TRUE, dpi = 600, bg = "white")
...
#ggsave("../figures/Fe_S_manuscript/S_pcoa4panel_SFO.png", plot = S_pcoa_4panel_SFO, device=NULL, path=NULL, scale=1, height = 7, width = 12, limitsize = TRUE, dpi = 600)
```
...
#ggsave("../figures/Fe_S_manuscript/FeGenieTotal_2panel.png", plot = FeGenieTotal_2panel, device=NULL, path=NULL, scale=1, height = 10, width = 7, limitsize = TRUE, dpi = 600)
```
...
vec_FeReduction_SFO <- as.data.frame(fit_FeReduction_SFO$vectors$arrows*sqrt(fit_FeReduction_SFO$vectors$r)*0.15) # adjusting to dodge points
...
# Add column with labels, for plotting:
vec_FeReduction_SFO$labels <- c("%C", "%N", "P", "Ca", "Cu", "CEC")

# Add a column with nudges for nudge_x and nudge_y:
vec_FeReduction_SFO$X_nudge <- c(-0.01, 0.03, 0.007, 0.05, -0.04, 0.02)
vec_FeReduction_SFO$Y_nudge <- c(0.02, 0.015, -0.01, -0.01, 0.03, -0.015)
...
plot7b <- ggplot(df7b, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
 ...
 geom_text(data=vec_FeReduction_SFO, aes(x=V1, y=V2), label=vec_FeReduction_SFO$labels, nudge_x = vec_FeReduction_SFO$X_nudge, nudge_y = vec_FeReduction_SFO$Y_nudge, size=4, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
explainvar1_SO4_red_combined_FIN # 36.7
explainvar2_SO4_red_combined_FIN # 22.4
...
# Add a column with nudges for nudge_x and nudge_y:
vec_SO4_red_combined_SFO$X_nudge <- c(0, -0.005, 0, 0.005, 0, -0.005, 0.005, 0)
vec_SO4_red_combined_SFO$Y_nudge <- c(0.007, 0, 0.007, 0, 0.005, 0, 0, -0.005)
...
plot7d <- ggplot(df7d, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_text(data=vec_SO4_red_combined_SFO, aes(x=V1, y=V2), label=vec_SO4_red_combined_SFO$labels, nudge_x = vec_SO4_red_combined_SFO$X_nudge, nudge_y = vec_SO4_red_combined_SFO$Y_nudge, size=4, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
# panel and add legend grob
FeRed_SO4Red_4panel <- ggarrange(plot7a, plot7b, plot7c, plot7d, ncol = 2, nrow = 2, labels = c("A", "B", "C", "D"), legend.grob = leg, legend = "right", align = "hv")

FeRed_SO4Red_4panel

#ggsave("../figures/Fe_S_manuscript/FeRed_SO4Red_4panel.png", plot = FeRed_SO4Red_4panel, device=NULL, path=NULL, scale=1, height = 7, width = 13, limitsize = TRUE, dpi = 600)

#ggsave("../figures/Fe_S_manuscript/Figure4.tiff", plot = FeRed_SO4Red_4panel, device="tiff", path=NULL, scale=1, height = 7, width = 13, limitsize = TRUE, dpi = 600, bg = "white")
```

```{r PCOA: Methane Oxidation}
# Classical (Metric) Multidimensional Scaling; returns PCoA coordinates
# eig=TRUE returns eigenvalues; k = # of dimensions to calculate
pcoa_CH4_ox <- cmdscale(CH4_oxBC, k=2, eig=TRUE, add=TRUE)

explainvar1b_CH4_ox <- round(pcoa_CH4_ox$eig[1] / sum(pcoa_CH4_ox$eig), 3) * 100
explainvar2b_CH4_ox <- round(pcoa_CH4_ox$eig[2] / sum(pcoa_CH4_ox$eig), 3) * 100

explainvar1b_CH4_ox # 27.8
explainvar2b_CH4_ox # 21.2

pcoa_CH4_ox2.groups <- paste(CH4_oxRA_design$Treat, CH4_oxRA_design$Plant, CH4_oxRA_design$Hist, sep = "_")
pcoa_CH4_ox2.points <- data.frame(pcoa_CH4_ox$points, group = pcoa_CH4_ox2.groups)

# Calculate Centroids (mean and SE)
pcoa_CH4_ox2.L.centroids <- reshape2::melt(pcoa_CH4_ox2.points, id="group", measure.vars = c("X1", "X2"))
pcoa_CH4_ox2.centroids <- acast(pcoa_CH4_ox2.L.centroids, variable ~ group, mean)
pcoa_CH4_ox2.centroids.se <- acast(pcoa_CH4_ox2.L.centroids, variable ~ group, se)
pcoa_CH4_ox2.centroids.sd <- acast(pcoa_CH4_ox2.L.centroids, variable ~ group, sd)

# Combine
pcoa_CH4_ox2.cent.dataframe <- cbind(t(pcoa_CH4_ox2.centroids), t(pcoa_CH4_ox2.centroids.se))
colnames(pcoa_CH4_ox2.cent.dataframe) <- c("V1", "V2", "V1e", "V2e")
...
pcoa_CH4_ox2.cent.dataframe.trts <- as.data.frame(pcoa_CH4_ox2.cent.dataframe) 
pcoa_CH4_ox2.cent.dataframe.trts$trt <- as.factor(trt_H)
pcoa_CH4_ox2.cent.dataframe.trts$plant <- as.factor(plant_H)
pcoa_CH4_ox2.cent.dataframe.trts$history <- as.factor(history)
pcoa_CH4_ox2.cent.dataframe.trts$history_plant<- as.factor(history_plant_combined)
pcoa_CH4_ox2.cent.dataframe.trts$centroid_or_sample <- "centroid"

# Add in points to data frame:
pcoa_CH4_ox2.points_ForPlot <- pcoa_CH4_ox2.points
colnames(pcoa_CH4_ox2.points_ForPlot) <- c("V1", "V2", "group")
# Add labels to match ...cent.dataframe.trts
pcoa_CH4_ox2.points_ForPlot$trt <- CH4_oxRA_design$Treat %>% str_replace_all(c("w" = "Wet", "_pre" = "Baseline", "d" = "Dry"))
pcoa_CH4_ox2.points_ForPlot$plant <- CH4_oxRA_design$Plant %>% str_replace_all(c("n" = "No Plant", "y" = "Plant", "_pre" = "Baseline"))
pcoa_CH4_ox2.points_ForPlot$history <- CH4_oxRA_design$Hist %>% str_replace_all(c("d" = "Dry", "w" = "Wet"))
pcoa_CH4_ox2.points_ForPlot$history_plant <- c(paste(pcoa_CH4_ox2.points_ForPlot$history, pcoa_CH4_ox2.points_ForPlot$plant, sep = "_"))
#remove "group" column
pcoa_CH4_ox2.points_ForPlot <- pcoa_CH4_ox2.points_ForPlot[, -3]
#add V1e and V2e as zeros:
pcoa_CH4_ox2.points_ForPlot$V1e <- numeric(24)
pcoa_CH4_ox2.points_ForPlot$V2e <- numeric(24)
# Add column to distinguish sample points from centroids, and later use to set alpha in plot
pcoa_CH4_ox2.points_ForPlot$centroid_or_sample <- "sample"

#Merge:
CH4_ox_centroids_points_merged <- rbind(pcoa_CH4_ox2.cent.dataframe.trts, pcoa_CH4_ox2.points_ForPlot)

# Add points data frame
pcoa_CH4_ox_DF <- as.data.frame(pcoa_CH4_ox$points)
```

```{r CH4_ox envfit and PCoA}
#Run envfit with new Soil data (SSF = Soil Start & Finish):
fit_CH4_ox_SSF <- envfit(pcoa_CH4_ox_DF, SoilGHG_StartFinish_Envfit, permutations = 10000, set.seed(42))
fit_CH4_ox_SSF

A_CH4_ox_SSF <-as.list(fit_CH4_ox_SSF$vectors)
vec_CH4_ox_SSF <- as.data.frame(fit_CH4_ox_SSF$vectors$arrows*sqrt(fit_CH4_ox_SSF$vectors$r)*0.15)
p_CH4_ox_SSF <- as.data.frame(A_CH4_ox_SSF$pvals)
vec_CH4_ox_SSF <- cbind(vec_CH4_ox_SSF, p_CH4_ox_SSF)
vec_CH4_ox_SSF <- subset(vec_CH4_ox_SSF, A_CH4_ox_SSF$pvals<=0.05)



#Plot using ggplot2
df8a <- as.data.frame(CH4_ox_centroids_points_merged)

plot8a <- ggplot(df8a, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  geom_errorbar(aes(ymax=V2+V2e, ymin=V2-V2e, width=0.003), colour="black") +   
  geom_point(aes(shape = history_plant, colour=trt, alpha = centroid_or_sample, size = centroid_or_sample), stroke=2) +  
  theme_bw() +
  scale_size_manual(values = c(6, 3), name = "Centroid or Sample")+
  ...
  scale_color_manual(values=c("black", "#997700", "#004488"), name="Treatment") +
  scale_shape_manual(values = c(7, 0, 15, 13, 1, 19), name="History_Plant") +
  xlab(glue("PCoA 1 ({explainvar1b_CH4_ox}%)")) + ylab(glue("PCoA 2 ({explainvar2b_CH4_ox}%)")) + 
  geom_segment(data=vec_CH4_ox_SSF, aes(x=0,xend=V1,y=0,yend=V2), linewidth=1, arrow = arrow(length = unit(0.2, "cm")),colour="black", alpha = 0.4, inherit.aes=F)+ 
 geom_text(data=vec_CH4_ox_SSF, aes(x=V1, y=V2), label= vec_CH4_ox_SSF$vec_labels, size=4, nudge_x = vec_CH4_ox_SSF$X_nudge, nudge_y = vec_CH4_ox_SSF$Y_nudge, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
```{r DBPLSR barplots 2}
# read in formatted table of dbplsr component 2 adjusted R2 results:
dbplsr_comp2_dec <- read.csv("~/GitHub/WetlandMesocosm_GHG_Timberlake/data/dbplsr_comp2_dec.csv")

methane.dbplsr.Fig <- ggplot(data = dbplsr_comp2_dec, aes(x = Timepoint, y = GHG, fill = Comp2_adjR2)) +
  geom_tile() +
  facet_wrap(~Dist_matrix) +
  ...
  theme(axis.ticks.length = unit(0.3, "cm")) + 
  labs(x = "", y = "Methane Flux", title = "") +
  theme(rect = element_rect(fill = "transparent")) +
  theme(plot.background = element_rect(color = NA)) +
  theme(axis.text.y = element_blank(),  # Remove y-axis text
        axis.ticks.y = element_blank()) + # Remove y-axis ticks
  scale_x_discrete(expand = c(0, 0))+  # Remove x-axis margin
  scale_y_discrete(expand = c(0, 0))  # Remove y-axis margin
...
# read in formatted table of dbplsr component 2 adjusted R2 results:
dbplsr_comp2_envfit <- read.csv("~/GitHub/WetlandMesocosm_GHG_Timberlake/data/dbplsr_comp2_envfit.csv")

methane.dbplsr.envfit <- ggplot(data = dbplsr_comp2_envfit, aes(x = Timepoint, y = Stat, fill = R2)) +
  geom_tile() +
  facet_wrap(~Dist_matrix) +
  scale_fill_viridis_c(option = "viridis") +  # Use viridis color palette
  theme_bw() +
  # Remove plot grid lines
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  # Set axis title and text properties, tick marks, and labels
  theme(text = element_text(size = 14),
        ...
        legend.text = element_text(size = 14)) + 
  theme(axis.ticks.length = unit(0.3, "cm")) + 
  labs(x = "", y = "", title = "", fill = expression(R^2 ~ "(CH"[4]~" flux ~ KEGG module)")) +
  theme(rect = element_rect(fill = "transparent")) +
  theme(plot.background = element_rect(color = NA)) +
 # theme(axis.text.y = element_blank(),  # Remove y-axis text
  #      axis.ticks.y = element_blank()) + # Remove y-axis ticks
...
methane.dbplsr.envfit

#ggsave("../figures/Fe_S_manuscript/methane_dbplsr_envfit_heatmap.png", plot=methane.dbplsr.envfit, device="png", path=NULL, scale=1, width=14, height=4, dpi=600, limitsize=TRUE, bg = "white")
```
...
# mantel tests:
vegan::mantel(FeReductionBC, SO4_red_combinedBC, permutations = 10000, set.seed(42))

vegan::mantel(FeReductionBC, Assim_SO4_redBC, permutations = 10000, set.seed(42))

vegan::mantel(FeReductionBC, Dissim_SO4_redBC, permutations = 10000, set.seed(42))

vegan::mantel(denitBC, ThioSOXBC, permutations = 10000, set.seed(42))

vegan::mantel(DNRABC, ThioSOXBC, permutations = 10000, set.seed(42))

vegan::mantel(ANRABC, ThioSOXBC, permutations = 10000, set.seed(42))

# Sulfate reduction ~ Methanogen
vegan::mantel(methBC, SO4_red_combinedBC, permutations = 10000, set.seed(42))

vegan::mantel(methBC, Assim_SO4_redBC, permutations = 10000, set.seed(42))

vegan::mantel(methBC, Dissim_SO4_redBC, permutations = 10000, set.seed(42))
```

```{r Mantel heatmap figure (overall comparisons)}
# List of distance matrices
matrices <- list(
  "Iron reduction" = FeReductionBC,
  "Assimilatory\n sulfate reduction" = Assim_SO4_redBC,
  "Dissimilatory\n sulfate reduction" = Dissim_SO4_redBC,
  "Sox pathway" = ThioSOXBC,
  "Denitrification" = denitBC,
  "DNRA" = DNRABC,
  Methanogenesis = methBC
)

# Initialize matrices to store r and p-values
n <- length(matrices)
r_matrix <- matrix(NA, n, n)
p_matrix <- matrix(NA, n, n)
rownames(r_matrix) <- colnames(r_matrix) <- names(matrices)
rownames(p_matrix) <- colnames(p_matrix) <- names(matrices)

# Set the seed ONCE before the loops start
set.seed(42)

# Run Mantel tests
for (i in 1:n) {
  for (j in 1:n) {
    test <- vegan::mantel(matrices[[i]], matrices[[j]], permutations = 9999)
    r_matrix[i, j] <- test$statistic
    p_matrix[i, j] <- test$signif
  }
}

# Melt matrices for ggplot
r_melt <- melt(r_matrix)
p_melt <- melt(p_matrix)
combined <- data.frame(
  Var1 = r_melt$Var1,
  Var2 = r_melt$Var2,
  r = round(r_melt$value, 2),
  p = p_melt$value
)

# Create label
combined$label <- mapply(function(r, p) {
  if (p < 0.01) {
    sci <- formatC(p, format = "e", digits = 1)  # e.g., "8.0e-03"
    parts <- strsplit(sci, "e")[[1]]
    coeff <- parts[1]
    exponent <- as.numeric(parts[2])
    paste0("atop(italic(r)==", formatC(r, digits = 2, format = "f"),
           ", italic(p)==", coeff, " %*% 10^", exponent, ")")
  } else {
    paste0("atop(italic(r)==", formatC(r, digits = 2, format = "f"),
           ", italic(p)==", formatC(p, format = "f", digits = 2), ")")
  }
}, combined$r, combined$p)

# Define consistent matrix order
matrix_names <- names(matrices)

# Set both Var1 and Var2 to the same factor levels
combined$Var1 <- factor(combined$Var1, levels = matrix_names)
combined$Var2 <- factor(combined$Var2, levels = matrix_names)

# Mask upper triangle: Var1 is column (X), Var2 is row (Y)
combined$mask <- match(combined$Var1, matrix_names) > match(combined$Var2, matrix_names)

# Flag diagonal cells
combined$is_diagonal <- as.character(combined$Var1) == as.character(combined$Var2)

# Set text color: black for diagonal, white otherwise
combined$text_color <- ifelse(combined$is_diagonal, "black", "white")

# Plot heatmap with masked lower triangle
Mantel_heat_overall <- ggplot(combined, aes(Var1, Var2)) +
  geom_tile(data = subset(combined, mask), fill = "white", color = "white") +
  geom_tile(data = subset(combined, !mask), aes(fill = r), color = "white") +
  geom_text(data = subset(combined, !mask), aes(label = label, color = text_color), size = 4, parse = TRUE) +
  scale_color_identity() +
  scale_fill_viridis_c(option = "viridis", name = "Mantel r", begin = 0, end = 1) +
  scale_y_discrete(limits = rev(matrix_names)) +  # reverse Y-axis
  scale_x_discrete(limits = matrix_names) +  # non reversed X-axis
  theme_minimal() +
  labs(x = "", y = "", title = "") +
  ...
  axis.line = element_blank()          # optional: remove axis lines
)

Mantel_heat_overall

#ggsave("../figures/Fe_S_manuscript/Mantel_heat_overall.tiff", plot=Mantel_heat_overall, device="tiff", path=NULL, scale=1, width=10, height=6, dpi=600, limitsize=TRUE ,bg="white")

#ggsave("../figures/Fe_S_manuscript/Mantel_heat_overall.png", plot=Mantel_heat_overall, device="png", path=NULL, scale=1, width=10, height=6, dpi=600, limitsize=TRUE ,bg="white")
```

#### Hydrology-specific Mantel tests
...
```{r Wrangling sulfate reduction and methanogen}
#Filter ASR, DSR, and Methanogen for wet history only:
Assim_SO4_redRA_WetHist_design <- Assim_SO4_redRA_design %>% dplyr::filter(Hist == "w")

Dissim_SO4_redRA_WetHist_design <- Dissim_SO4_redRA_design %>% dplyr::filter(Hist == "w")

methRA_WetHist_design <- methRA_design %>% dplyr::filter(Hist == "w")

#Filter ASR, DSR, and Methanogen for dry history only:
Assim_SO4_redRA_DryHist_design <- Assim_SO4_redRA_design %>% dplyr::filter(Hist == "d")

Dissim_SO4_redRA_DryHist_design <- Dissim_SO4_redRA_design %>% dplyr::filter(Hist == "d")

methRA_DryHist_design <- methRA_design %>% dplyr::filter(Hist == "d")

#Filter ASR, DSR, and Methanogen for wet treatment only:
Assim_SO4_redRA_WetTreat_design <- Assim_SO4_redRA_design %>% dplyr::filter(Treat == "w")

Dissim_SO4_redRA_WetTreat_design <- Dissim_SO4_redRA_design %>% dplyr::filter(Treat == "w")

methRA_WetTreat_design <- methRA_design %>% dplyr::filter(Treat == "w")

#Filter ASR, DSR, and Methanogen for dry treatment only:
Assim_SO4_redRA_DryTreat_design <- Assim_SO4_redRA_design %>% dplyr::filter(Treat == "d")

Dissim_SO4_redRA_DryTreat_design <- Dissim_SO4_redRA_design %>% dplyr::filter(Treat == "d")

methRA_DryTreat_design <- methRA_design %>% dplyr::filter(Treat == "d")

# Make Bray-Curtis distance matrices for numeric only data in new RA dfs:
Assim_SO4_redRA_WetHist_BC <- Assim_SO4_redRA_WetHist_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

Dissim_SO4_redRA_WetHist_BC <- Dissim_SO4_redRA_WetHist_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

methRA_WetHist_BC <- methRA_WetHist_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

Assim_SO4_redRA_DryHist_BC <- Assim_SO4_redRA_DryHist_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

Dissim_SO4_redRA_DryHist_BC <- Dissim_SO4_redRA_DryHist_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

methRA_DryHist_BC <- methRA_DryHist_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

Assim_SO4_redRA_WetTreat_BC <- Assim_SO4_redRA_WetTreat_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

Dissim_SO4_redRA_WetTreat_BC <- Dissim_SO4_redRA_WetTreat_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

methRA_WetTreat_BC <- methRA_WetTreat_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

Assim_SO4_redRA_DryTreat_BC <- Assim_SO4_redRA_DryTreat_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

Dissim_SO4_redRA_DryTreat_BC <- Dissim_SO4_redRA_DryTreat_design[,-c(1:3)] %>%
  vegdist(method = 'bray')

methRA_DryTreat_BC <- methRA_DryTreat_design[,-c(1:3)] %>%
  vegdist(method = 'bray')
```

```{r Hydrology-specific Mantel tests Fe red and combined sulfate red}
# Fe reduction (FeGenie) and Sulfate reduction (combined) in Wet history:
vegan::mantel(FeReductionRA_WetHist_BC, SO4_red_combinedRA_WetHist_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (combined) in Dry history:
vegan::mantel(FeReductionRA_DryHist_BC, SO4_red_combinedRA_DryHist_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (combined) in Wet treatment:
vegan::mantel(FeReductionRA_WetTreat_BC, SO4_red_combinedRA_WetTreat_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (combined) in Dry treatment:
vegan::mantel(FeReductionRA_DryTreat_BC, SO4_red_combinedRA_DryTreat_BC, permutations = 10000, set.seed(42))
```

```{r Hydrology-specific Mantel tests Fe red and Assim sulfate red}
# Fe reduction (FeGenie) and Sulfate reduction (assimilatory) in Wet history:
vegan::mantel(FeReductionRA_WetHist_BC, Assim_SO4_redRA_WetHist_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (assimilatory) in Dry history:
vegan::mantel(FeReductionRA_DryHist_BC, Assim_SO4_redRA_DryHist_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (assimilatory) in Wet treatment:
vegan::mantel(FeReductionRA_WetTreat_BC, Assim_SO4_redRA_WetTreat_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (assimilatory) in Dry treatment:
vegan::mantel(FeReductionRA_DryTreat_BC, Assim_SO4_redRA_DryTreat_BC, permutations = 10000, set.seed(42))
```

```{r Hydrology-specific Mantel tests Fe red and Dissim sulfate red}
# Fe reduction (FeGenie) and Sulfate reduction (dissimilatory) in Wet history:
vegan::mantel(FeReductionRA_WetHist_BC, Dissim_SO4_redRA_WetHist_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (dissimilatory) in Dry history:
vegan::mantel(FeReductionRA_DryHist_BC, Dissim_SO4_redRA_DryHist_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (dissimilatory) in Wet treatment:
vegan::mantel(FeReductionRA_WetTreat_BC, Dissim_SO4_redRA_WetTreat_BC, permutations = 10000, set.seed(42))

# Fe reduction (FeGenie) and Sulfate reduction (dissimilatory) in Dry treatment:
vegan::mantel(FeReductionRA_DryTreat_BC, Dissim_SO4_redRA_DryTreat_BC, permutations = 10000, set.seed(42))
```

```{r Hydrology-specific Mantel tests sulfate red and Methanogen}
# Methanogenesis and sulfate reduction in Wet history:
vegan::mantel(methRA_WetHist_BC, SO4_red_combinedRA_WetHist_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_WetHist_BC, Assim_SO4_redRA_WetHist_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_WetHist_BC, Dissim_SO4_redRA_WetHist_BC, permutations = 10000, set.seed(42))

# Methanogenesis and sulfate reduction in Dry history:
vegan::mantel(methRA_DryHist_BC, SO4_red_combinedRA_DryHist_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_DryHist_BC, Assim_SO4_redRA_DryHist_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_DryHist_BC, Dissim_SO4_redRA_DryHist_BC, permutations = 10000, set.seed(42))

# Methanogenesis and sulfate reduction in Wet treatment:
vegan::mantel(methRA_WetTreat_BC, SO4_red_combinedRA_WetTreat_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_WetTreat_BC, Assim_SO4_redRA_WetTreat_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_WetTreat_BC, Dissim_SO4_redRA_WetTreat_BC, permutations = 10000, set.seed(42))

# Methanogenesis and sulfate reduction in Dry treatment:
vegan::mantel(methRA_DryTreat_BC, SO4_red_combinedRA_DryTreat_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_DryTreat_BC, Assim_SO4_redRA_DryTreat_BC, permutations = 10000, set.seed(42))

vegan::mantel(methRA_DryTreat_BC, Dissim_SO4_redRA_DryTreat_BC, permutations = 10000, set.seed(42))
```

```{r Mantel matrix: Wet History}
# Distance matrices, wet history only
matrices <- list(
  "Iron reduction" = FeReductionRA_WetHist_BC,
  "Assimilatory\n sulfate reduction" = Assim_SO4_redRA_WetHist_BC,
  "Dissimilatory\n sulfate reduction" = Dissim_SO4_redRA_WetHist_BC,
  Methanogenesis = methRA_WetHist_BC
)
...
# Create label
combined1$label <- mapply(function(r, p) {
  if (p < 0.01) {
    sci <- formatC(p, format = "e", digits = 1)  # e.g., "8.0e-03"
    parts <- strsplit(sci, "e")[[1]]
    coeff <- parts[1]
...
}, combined1$r, combined1$p)

# Define consistent matrix order
matrix_names <- names(matrices)

# Set both Var1 and Var2 to the same factor levels
combined1$Var1 <- factor(combined1$Var1, levels = matrix_names)
combined1$Var2 <- factor(combined1$Var2, levels = matrix_names)

# Mask upper triangle: Var1 is column (X), Var2 is row (Y)
combined1$mask <- match(combined1$Var1, matrix_names) > match(combined1$Var2, matrix_names)

# Flag diagonal cells
combined1$is_diagonal <- as.character(combined1$Var1) == as.character(combined1$Var2)

# Set text color: black for diagonal, white otherwise
combined1$text_color <- ifelse(combined1$is_diagonal, "black", "white")
```

```{r Mantel heatmap: Dry Hist}
# Distance matrices, dry history only
matrices <- list(
  "Iron reduction" = FeReductionRA_DryHist_BC,
  "Assimilatory\n sulfate reduction" = Assim_SO4_redRA_DryHist_BC,
  "Dissimilatory\n sulfate reduction" = Dissim_SO4_redRA_DryHist_BC,
  Methanogenesis = methRA_DryHist_BC
)
...
# Create label
combined2$label <- mapply(function(r, p) {
  if (p < 0.01) {
    sci <- formatC(p, format = "e", digits = 1)  # e.g., "8.0e-03"
    parts <- strsplit(sci, "e")[[1]]
    coeff <- parts[1]
...
}, combined2$r, combined2$p)

# Define consistent matrix order
matrix_names <- names(matrices)

# Set both Var1 and Var2 to the same factor levels
combined2$Var1 <- factor(combined2$Var1, levels = matrix_names)
combined2$Var2 <- factor(combined2$Var2, levels = matrix_names)

# Mask upper triangle: Var1 is column (X), Var2 is row (Y)
combined2$mask <- match(combined2$Var1, matrix_names) > match(combined2$Var2, matrix_names)

# Flag diagonal cells
combined2$is_diagonal <- as.character(combined2$Var1) == as.character(combined2$Var2)

# Set text color: black for diagonal, white otherwise
combined2$text_color <- ifelse(combined2$is_diagonal, "black", "white")
```

```{r Mantel heatmap: Wet Treatment}
# Distance matrices, wet treatment only
matrices <- list(
  "Iron reduction" = FeReductionRA_WetTreat_BC,
  "Assimilatory\n sulfate reduction" = Assim_SO4_redRA_WetTreat_BC,
  "Dissimilatory\n sulfate reduction" = Dissim_SO4_redRA_WetTreat_BC,
  Methanogenesis = methRA_WetTreat_BC
)
...
# Create label
combined3$label <- mapply(function(r, p) {
  if (p < 0.01) {
    sci <- formatC(p, format = "e", digits = 1)  # e.g., "8.0e-03"
    parts <- strsplit(sci, "e")[[1]]
    coeff <- parts[1]
...
}, combined3$r, combined3$p)

# Define consistent matrix order
matrix_names <- names(matrices)

# Set both Var1 and Var2 to the same factor levels
combined3$Var1 <- factor(combined3$Var1, levels = matrix_names)
combined3$Var2 <- factor(combined3$Var2, levels = matrix_names)

# Mask upper triangle: Var1 is column (X), Var2 is row (Y)
combined3$mask <- match(combined3$Var1, matrix_names) > match(combined3$Var2, matrix_names)

# Flag diagonal cells
combined3$is_diagonal <- as.character(combined3$Var1) == as.character(combined3$Var2)

# Set text color: black for diagonal, white otherwise
combined3$text_color <- ifelse(combined3$is_diagonal, "black", "white")
```

```{r Mantel heatmap: Dry Treatment}
# Distance matrices, dry treatment only
matrices <- list(
  "Iron reduction" = FeReductionRA_DryTreat_BC,
  "Assimilatory\n sulfate reduction" = Assim_SO4_redRA_DryTreat_BC,
  "Dissimilatory\n sulfate reduction" = Dissim_SO4_redRA_DryTreat_BC,
  Methanogenesis = methRA_DryTreat_BC
)
...
# Create label
combined4$label <- mapply(function(r, p) {
  if (p < 0.01) {
    sci <- formatC(p, format = "e", digits = 1)  # e.g., "8.0e-03"
    parts <- strsplit(sci, "e")[[1]]
    coeff <- parts[1]
...
}, combined4$r, combined4$p)

# Define consistent matrix order
matrix_names <- names(matrices)

# Set both Var1 and Var2 to the same factor levels
combined4$Var1 <- factor(combined4$Var1, levels = matrix_names)
combined4$Var2 <- factor(combined4$Var2, levels = matrix_names)

# Mask upper triangle: Var1 is column (X), Var2 is row (Y)
combined4$mask <- match(combined4$Var1, matrix_names) > match(combined4$Var2, matrix_names)

# Flag diagonal cells
combined4$is_diagonal <- as.character(combined4$Var1) == as.character(combined4$Var2)

# Set text color: black for diagonal, white otherwise
combined4$text_color <- ifelse(combined4$is_diagonal, "black", "white")
```

```{r Global r min and r max for viridis scale}
# Step 1: Get global min and max r values
global_r_min <- min(c(combined1$r, combined2$r, combined3$r, combined4$r), na.rm = TRUE)
global_r_max <- max(c(combined1$r, combined2$r, combined3$r, combined4$r), na.rm = TRUE)
```

```{r Heatmap 1: Wet History}
# Plot heatmap with masked lower triangle
MantelHeatHydro_1 <- ggplot(combined1, aes(Var1, Var2)) +
  geom_tile(data = subset(combined1, mask), fill = "white", color = "white") +
  geom_tile(data = subset(combined1, !mask), aes(fill = r), color = "white") +
  geom_text(data = subset(combined1, !mask), aes(label = label, color = text_color), size = 4, parse = TRUE) +
  scale_color_identity()+  # use actual color values from the data
  scale_fill_viridis_c(option = "viridis", name = "Mantel r", limits = c(global_r_min, global_r_max)) +
  scale_y_discrete(limits = rev(matrix_names)) +  # reverse Y-axis
  scale_x_discrete(limits = matrix_names) +  # non reversed X-axis
  theme_minimal() +
  labs(x = "", y = "", title = "Wet History") +
  #theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r Heatmap 2: Dry History}
# Plot heatmap with masked lower triangle
MantelHeatHydro_2 <- ggplot(combined2, aes(Var1, Var2)) +
  geom_tile(data = subset(combined2, mask), fill = "white", color = "white") +
  geom_tile(data = subset(combined2, !mask), aes(fill = r), color = "white") +
  geom_text(data = subset(combined2, !mask), aes(label = label, color = text_color), size = 4, parse = TRUE) +
  scale_color_identity()+  # use actual color values from the data
  scale_fill_viridis_c(option = "viridis", name = "Mantel r", limits = c(global_r_min, global_r_max)) +
  scale_y_discrete(limits = rev(matrix_names)) +  # reverse Y-axis
  scale_x_discrete(limits = matrix_names) +  # non reversed X-axis
  theme_minimal() +
  labs(x = "", y = "", title = "Dry History") +
  #theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r Heatmap 3: Wet Treatment}
# Plot heatmap with masked lower triangle
MantelHeatHydro_3 <- ggplot(combined3, aes(Var1, Var2)) +
  geom_tile(data = subset(combined3, mask), fill = "white", color = "white") +
  geom_tile(data = subset(combined3, !mask), aes(fill = r), color = "white") +
  geom_text(data = subset(combined3, !mask), aes(label = label, color = text_color), size = 4, parse = TRUE) +
  scale_color_identity()+  # use actual color values from the data
  scale_fill_viridis_c(option = "viridis", name = "Mantel r", limits = c(global_r_min, global_r_max)) +
  scale_y_discrete(limits = rev(matrix_names)) +  # reverse Y-axis
  scale_x_discrete(limits = matrix_names) +  # non reversed X-axis
  theme_minimal() +
  labs(x = "", y = "", title = "Wet Treatment") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
  axis.text = element_text(size = 12, face = "bold"),
  axis.ticks.x = element_blank(),
  axis.ticks.y = element_blank(),
...
```{r Heatmap 4: Dry Treatment}
# Plot heatmap with masked lower triangle
MantelHeatHydro_4 <- ggplot(combined4, aes(Var1, Var2)) +
  geom_tile(data = subset(combined4, mask), fill = "white", color = "white") +
  geom_tile(data = subset(combined4, !mask), aes(fill = r), color = "white") +
  geom_text(data = subset(combined4, !mask), aes(label = label, color = text_color), size = 4, parse = TRUE) +
  scale_color_identity()+  # use actual color values from the data
  scale_fill_viridis_c(option = "viridis", name = "Mantel r", limits = c(global_r_min, global_r_max)) +
  scale_y_discrete(limits = rev(matrix_names)) +  # reverse Y-axis
  scale_x_discrete(limits = matrix_names) +  # non reversed X-axis
  theme_minimal() +
  labs(x = "", y = "", title = "Dry Treatment") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
  axis.text = element_text(size = 12, face = "bold"),
...
```{r 4-panel Hydrology Mantel Heatmaps}
MantelHeatHydro_ALL <- ggarrange(MantelHeatHydro_1, MantelHeatHydro_2, MantelHeatHydro_3, MantelHeatHydro_4, labels = c("A", "B", "C", "D"), ncol = 2, nrow = 2, legend = "right", common.legend = T, align = "hv")

MantelHeatHydro_ALL

#ggsave("../figures/Fe_S_manuscript/MantelHeatHydro.tiff", plot=MantelHeatHydro_ALL, device="tiff", path=NULL, scale=1, width=13, height=9, dpi=600, limitsize=TRUE ,bg="white")

#ggsave("../figures/Fe_S_manuscript/MantelHeatHydro.png", plot=MantelHeatHydro_ALL, device="png", path=NULL, scale=1, width=13, height=9, dpi=600, limitsize=TRUE ,bg="white")
```
