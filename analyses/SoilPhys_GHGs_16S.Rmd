...
soil_nofield$IRIS_percent <- soil_nofield$redox.percent.paint.removed*100
...
```{r IRIS Kruskal and SDCFlig and Wilcox}
# Kruskal start, Plant:
kruskal_IRIS_start <- kruskal.test(data = soil_nofield.Start, IRIS_percent ~ treatment)
kruskal_IRIS_start

  # Pairwise comparisons
SDCFlig_IRIS_start <- pSDCFlig(x = (soil_nofield.Start$IRIS_percent), g = soil_nofield.Start$treatment, method = "Monte Carlo", n.mc = 10000)
SDCFlig_IRIS_start

# Kruskal finish:
kruskal_IRIS_finish <- kruskal.test(data = soil_nofield.Finish, IRIS_percent ~ treatment)
kruskal_IRIS_finish

  # Pairwise comparisons
SDCFlig_IRIS_finish <- pSDCFlig(x = (soil_nofield.Finish$IRIS_percent), g = soil_nofield.Finish$treatment, method = "Asymptotic", n.mc = 1000)
SDCFlig_IRIS_finish

# Do a Wilcox on Plant vs. No Plant just to verify that there is no difference
  # wilcox start
wilcox_IRIS_start <- compare_means(IRIS_percent~plant, soil_nofield.Start, method = "wilcox.test", paired = T)
wilcox_IRIS_start
  # wilcox finish
wilcox_IRIS_finish <- compare_means(IRIS_percent~plant, soil_nofield.Finish, method = "wilcox.test", paired = T)
wilcox_IRIS_finish
```
...
#Plotting:
#Start:
iris_tubes.Start <- ggplot(soil_nofield.Start, aes(x=treatment, y=IRIS_percent)) +
...
#Finish:
iris_tubes.Finish <- ggplot(soil_nofield.Finish, aes(x=treatment, y=IRIS_percent)) +
  geom_boxplot(outlier.shape = NA) +
  facet_grid(~plant) +
  geom_point(aes(color=factor(treatment)), size = 2, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), name="Treatment") +
...
```{r IRIS boxplot with stats for FeS manuscript}
# Set levels of time so facets are ordered correctly
soil_nofield$time <- factor(soil_nofield$time, levels = c("Start", "Finish"))

# Adjust labels
soil_nofield$time <- revalue(soil_nofield$time, c(Start = "Start (Week 0)", Finish = "Finish (Week 8)"))

# Multiple comparisons data frame to get started
IRIS_multComp_4plot <- compare_means(IRIS_percent~treatment, soil_nofield, group.by="time", method = "wilcox.test", paired = F, p.adjust.method = "fdr")

# Have to manually add finish Dry vs. Interim because all zeroes = not calculable
new_row <- data.frame(
  time = "Finish",
  .y. = "IRIS_percent",
  group1 = "Dry",
...
IRIS_multComp_4plot <- rbind(new_row, IRIS_multComp_4plot)

# Adjustments for plotting:
IRIS_multComp_4plot <- add_significance(IRIS_multComp_4plot, p.col = "p.adj", output.col = "p.adj.signif", cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, Inf), symbols = c("****", "***", "**", "*", "ns"))

# Turn into data frame:
IRIS_multComp_4plot <- as.data.frame(IRIS_multComp_4plot)

# Add on SDCFlig p-values:
IRIS_multComp_4plot$SDCF_p <- c(SDCFlig_IRIS_finish$p.val, SDCFlig_IRIS_start$p.val)
IRIS_multComp_4plot$SDCF_p[1] <- 1

# Symbols for plotting:
IRIS_multComp_4plot <- add_significance(IRIS_multComp_4plot, p.col = "SDCF_p", output.col = "p.SDCF.signif", cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, Inf), symbols = c("****", "***", "**", "*", "ns"))
...
# Plotting:
plot.IRIS_stats <- ggplot(soil_nofield, aes(x=treatment, y=IRIS_percent)) +
  facet_wrap(vars(as.factor(time)), nrow = 1, ncol = 2)+
  ...
  theme(text=element_text(size=20),axis.title=element_text(size=20,face="bold"),
          ...
          axis.title.y=element_text(margin=margin(r=10)),
          axis.text.x = element_text(angle = 0),panel.border = element_rect(colour = "black",size=1),strip.text = element_text(size = 20), legend.position="top", legend.title = element_text(size=20), legend.text=element_text(size=20)) + 
          theme(axis.ticks.length=unit(0.3,"cm")) + 
          labs(x = "", y = "IRIS percent", title="") +
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_pvalue_manual(data = IRIS_multComp_4plot, comparisons = list(c("Dry", "Interim"), c("Dry", "Wet"), c("Interim", "Wet")), label = "p.SDCF.signif", size = 15, vjust = 1, hide.ns = "p", y.position = max(soil_nofield$IRIS_percent), step.group.by = "time", step.increase = 0.1)
 
plot.IRIS_stats

#ggsave("../figures/Fe_S_manuscript/iris_stats.png", plot=plot.IRIS_stats, device=NULL, path=NULL, scale=1, width=9, height=5, dpi=600, limitsize=TRUE ,bg="white")

#ggsave("../figures/Fe_S_manuscript/iris_stats.tiff", plot=plot.IRIS_stats, device="tiff", path=NULL, scale=1, width=9, height=5, dpi=600, limitsize=TRUE ,bg="white")
```

```{r IRIS plot for Ecosphere Sept 2025 revision}
# Add history as shape. Remove stats. Add X-axis title: Treatment.

# To use open and closed shapes, must create a combined category for History and Plant:
soil_nofield$history_plant <- as.factor(c(paste(soil_nofield$history, soil_nofield$plant, sep = "_")))


# Plotting:
plot.IRIS_nested <- ggplot(soil_nofield, aes(x=treatment, y=IRIS_percent)) +
  facet_wrap(vars(as.factor(time)), nrow = 1, ncol = 2)+
                    geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                    geom_point(aes(color=treatment, shape=history_plant), size = 3, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
...
#ggsave("../figures/Ecosphere_pub/iris_tubes.tiff", plot=plot.IRIS_nested, device="tiff", path=NULL, scale=1, width=9, height=5, dpi=600, limitsize=TRUE, bg="white")

#ggsave("../figures/Ecosphere_pub/iris_tubes.png", plot=plot.IRIS_nested, device="png", path=NULL, scale=1, width=9, height=5, dpi=600, limitsize=TRUE, bg="white")
```
...
```
## Flux with T0: Kruskal and SDCFlig
...
kruskal_CH4_6.28_w_T0
  ...
  # Pairwise comparisons:
    # Must subset 6/28:
select_fluxes_combined_w_T0_6.28 <- select_fluxes_combined_w_T0 %>% filter(date_short == "6/28")
...
kruskal_CO2_6.28_w_T0
  ...
  # Pairwise comparisons:
SDCFlig_CO2_6.28_w_T0 <- pSDCFlig(x = select_fluxes_combined_w_T0_6.28$f0_CO2, g = select_fluxes_combined_w_T0_6.28$treatment, method = "Monte Carlo", n.mc = 10000)
...
kruskal_CO2_7.25_w_T0
  ...
  # Pairwise comparisons:
SDCFlig_CO2_7.25_w_T0 <- pSDCFlig(x = select_fluxes_combined_w_T0_7.25$f0_CO2, g = select_fluxes_combined_w_T0_7.25$treatment, method = "Monte Carlo", n.mc = 10000)
...
```{r wilcox tests long format}
# June 13th time point:
  # CH4:
CH4_wilcox_6.13 <- wilcox.test(select_fluxes_combined_w_T0_6.13$f0_CH4[select_fluxes_combined_w_T0_6.13$plant == "N"], select_fluxes_combined_w_T0_6.13$f0_CH4[select_fluxes_combined_w_T0_6.13$plant == "P"], data = select_fluxes_combined_w_T0_6.13, paired = T)
CH4_wilcox_6.13
 
 # CO2:
CO2_wilcox_6.13 <- wilcox.test(select_fluxes_combined_w_T0_6.13$f0_CO2[select_fluxes_combined_w_T0_6.13$plant == "N"], select_fluxes_combined_w_T0_6.13$f0_CO2[select_fluxes_combined_w_T0_6.13$plant == "P"], data = select_fluxes_combined_w_T0_6.13, paired = T)
CO2_wilcox_6.13

  # N2O:
N2O_wilcox_6.13 <- wilcox.test(select_fluxes_combined_w_T0_6.13$f0_N2O[select_fluxes_combined_w_T0_6.13$plant == "N"], select_fluxes_combined_w_T0_6.13$f0_N2O[select_fluxes_combined_w_T0_6.13$plant == "P"], data = select_fluxes_combined_w_T0_6.13, paired = T)
N2O_wilcox_6.13

# June 28th time point:
  # CH4:
CH4_wilcox_6.28 <- wilcox.test(select_fluxes_combined_w_T0_6.28$f0_CH4[select_fluxes_combined_w_T0_6.28$plant == "N"], select_fluxes_combined_w_T0_6.28$f0_CH4[select_fluxes_combined_w_T0_6.28$plant == "P"], data = select_fluxes_combined_w_T0_6.28, paired = T)
CH4_wilcox_6.28

  # CO2:
CO2_wilcox_6.28 <- wilcox.test(select_fluxes_combined_w_T0_6.28$f0_CO2[select_fluxes_combined_w_T0_6.28$plant == "N"], select_fluxes_combined_w_T0_6.28$f0_CO2[select_fluxes_combined_w_T0_6.28$plant == "P"], data = select_fluxes_combined_w_T0_6.28, paired = T)
CO2_wilcox_6.28

  # N2O:
N2O_wilcox_6.28 <- wilcox.test(select_fluxes_combined_w_T0_6.28$f0_N2O[select_fluxes_combined_w_T0_6.28$plant == "N"], select_fluxes_combined_w_T0_6.28$f0_N2O[select_fluxes_combined_w_T0_6.28$plant == "P"], data = select_fluxes_combined_w_T0_6.28, paired = T)
N2O_wilcox_6.28

# July 11th time point:
  # CH4:
CH4_wilcox_7.11 <- wilcox.test(select_fluxes_combined_w_T0_7.11$f0_CH4[select_fluxes_combined_w_T0_7.11$plant == "N"], select_fluxes_combined_w_T0_7.11$f0_CH4[select_fluxes_combined_w_T0_7.11$plant == "P"], data = select_fluxes_combined_w_T0_7.11, paired = T)
CH4_wilcox_7.11

  # CO2:
CO2_wilcox_7.11 <- wilcox.test(select_fluxes_combined_w_T0_7.11$f0_CO2[select_fluxes_combined_w_T0_7.11$plant == "N"], select_fluxes_combined_w_T0_7.11$f0_CO2[select_fluxes_combined_w_T0_7.11$plant == "P"], data = select_fluxes_combined_w_T0_7.11, paired = T)
CO2_wilcox_7.11

  # N2O:
N2O_wilcox_7.11 <- wilcox.test(select_fluxes_combined_w_T0_7.11$f0_N2O[select_fluxes_combined_w_T0_7.11$plant == "N"], select_fluxes_combined_w_T0_7.11$f0_N2O[select_fluxes_combined_w_T0_7.11$plant == "P"], data = select_fluxes_combined_w_T0_7.11, paired = T)
N2O_wilcox_7.11

# July 25th time point:
  # CH4:
CH4_wilcox_7.25 <- wilcox.test(select_fluxes_combined_w_T0_7.25$f0_CH4[select_fluxes_combined_w_T0_7.25$plant == "N"], select_fluxes_combined_w_T0_7.25$f0_CH4[select_fluxes_combined_w_T0_7.25$plant == "P"], data = select_fluxes_combined_w_T0_7.25, paired = T)
CH4_wilcox_7.25

  # CO2:
CO2_wilcox_7.25 <- wilcox.test(select_fluxes_combined_w_T0_7.25$f0_CO2[select_fluxes_combined_w_T0_7.25$plant == "N"], select_fluxes_combined_w_T0_7.25$f0_CO2[select_fluxes_combined_w_T0_7.25$plant == "P"], data = select_fluxes_combined_w_T0_7.25, paired = T)
CO2_wilcox_7.25

  # N2O:
N2O_wilcox_7.25 <- wilcox.test(select_fluxes_combined_w_T0_7.25$f0_N2O[select_fluxes_combined_w_T0_7.25$plant == "N"], select_fluxes_combined_w_T0_7.25$f0_N2O[select_fluxes_combined_w_T0_7.25$plant == "P"], data = select_fluxes_combined_w_T0_7.25, paired = T)
N2O_wilcox_7.25

# August 11th time point:
  # Make subset of select_fluxes_combined:
select_fluxes_combined_w_T0_8.11 <- select_fluxes_combined_w_T0 %>% filter(date_short == "8/11")

  # CH4:
CH4_wilcox_8.11 <- wilcox.test(select_fluxes_combined_w_T0_8.11$f0_CH4[select_fluxes_combined_w_T0_8.11$plant == "N"], select_fluxes_combined_w_T0_8.11$f0_CH4[select_fluxes_combined_w_T0_8.11$plant == "P"], data = select_fluxes_combined_w_T0_8.11, paired = T)
CH4_wilcox_8.11

  # CO2:
CO2_wilcox_8.11 <- wilcox.test(select_fluxes_combined_w_T0_8.11$f0_CO2[select_fluxes_combined_w_T0_8.11$plant == "N"], select_fluxes_combined_w_T0_8.11$f0_CO2[select_fluxes_combined_w_T0_8.11$plant == "P"], data = select_fluxes_combined_w_T0_8.11, paired = T)
CO2_wilcox_8.11

  # N2O:
N2O_wilcox_8.11 <- wilcox.test(select_fluxes_combined_w_T0_8.11$f0_N2O[select_fluxes_combined_w_T0_8.11$plant == "N"], select_fluxes_combined_w_T0_8.11$f0_N2O[select_fluxes_combined_w_T0_8.11$plant == "P"], data = select_fluxes_combined_w_T0_8.11, paired = T)
N2O_wilcox_8.11
```

### Plotting Plant vs. No Plant

```{r History_plant for "nested" symbols}
# Add history_plant for plotting:
select_fluxes_combined_w_T0$history_plant <- as.factor(c(paste(select_fluxes_combined_w_T0$history, select_fluxes_combined_w_T0$plant, sep = "_")))
```

```{r CH4 flux w T0 NP vs. P}
# Wilcox Test, nicer format, can use to verify stats on plot:
CH4_wilcox<- compare_means(f0_CH4~plant, select_fluxes_combined_w_T0, group.by="date_format", method = "wilcox.test", paired = T)
...
plot.CH4.flux.NPvP <- ggplot(select_fluxes_combined_w_T0, aes(x=plant, y=f0_CH4)) +
  facet_wrap(~date_format, nrow = 1, ncol = 5)+
                     ...
                     geom_point(aes(color=treatment, shape=history_plant), size = 2, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  scale_shape_manual(labels = c("Dry_No Plant", "Dry_Plant", "Interim_No Plant", "Interim_Plant", "Wet_No Plant", "Wet_Plant"), values = c(0, 15, 2, 17, 1, 19), name="History_Plant")+
  theme_bw() +
  #Remove plot grid lines
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  coord_cartesian(ylim = c(-16, 50)) +  # Use coord_cartesian to limit y-axis range
  stat_pvalue_manual(data = CH4_wilcox.DF, comparisons = list(c("N", "P")), label = "p.signif", size = 10, hide.ns = "p", y.position = 45, step.group.by = "date_format", bracket.size = 0) # Must use manual b/c outlier zoom in
...
```{r CO2 flux NP vs P}
# Wilcox Test, nicer format, can use to verify stats on plot:
CO2_wilcox<- compare_means(f0_CO2~plant, select_fluxes_combined_w_T0, group.by="date_format", method = "wilcox.test", paired = T)
...
```{r N2O flux NP vs. P}
# Wilcox Test, nicer format, can use to verify stats on plot:
N2O_wilcox<- compare_means(f0_N2O~plant, select_fluxes_combined_w_T0, group.by="date_format", method = "wilcox.test", paired = T)
...
CombPlot_flux_NPvP <- ggarrange(plot.CH4.flux.NPvP, plot.CO2.flux.NPvP, plot.N2O.flux.NPvP, labels = c("(a)", "(b)","(c)"), ncol = 1, nrow = 3, legend = "right", common.legend = T, align = "hv")

CombPlot_flux_NPvP

#ggsave("../figures/Ecosphere_pub/GHG_NPvP/CH4_N2O_suppFig.tiff", plot=CombPlot_flux_NPvP, device="tiff", scale=1, width=14, height=12, dpi=600, limitsize=TRUE, bg="white")

#ggsave("../figures/Ecosphere_pub/GHG_NPvP/CH4_N2O_suppFig.png", plot=CombPlot_flux_NPvP, device="png", scale=1, width=14, height=12, dpi=600, limitsize=TRUE, bg="white")
```
...
plot.CH4.flux.Trt <- ggplot(select_fluxes_combined_w_T0, aes(x=treatment, y=f0_CH4)) +
  ...
  theme(plot.background = element_rect(color=NA))+
  coord_cartesian(ylim = c(-16, 50)) +  # Use coord_cartesian to limit y-axis range
  stat_pvalue_manual(data = CH4_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = "p.adj", y.position = 45, step.group.by = "date_format", step.increase = 0.02, tip.length = 0.006)
...
plot.N2O.flux.Trt <- ggplot(select_fluxes_combined_w_T0, aes(x=treatment, y=f0_N2O)) +
  ...
  theme(text=element_text(size=20),axis.title=element_text(size=20,face="bold"),
          ...
          axis.text.x = element_text(angle = 45, hjust = 1), panel.border = element_rect(colour = "black",size=1), strip.text = element_text(size = 20), legend.position="right", legend.title = element_text(size=20), legend.text=element_text(size=20)) + 
          theme(axis.ticks.length=unit(0.3,"cm")) + 
          labs(x = "Treatment", y = expression(paste("mg N"[2],"O m"^{-2}, " h"^{-1})), title="") +
...
```{r Combined GHG Boxplot}
CombPlot_flux_treat <- ggarrange(plot.CH4.flux.Trt, plot.CO2.flux.Trt, plot.N2O.flux.Trt, labels = c("(a)", "(b)", "(c)"), ncol = 1, nrow = 3, legend = "right", common.legend = T, align = "hv")

CombPlot_flux_treat

#ggsave("../figures/Ecosphere_pub/GHG_Treatment/Figure3.tiff", plot=CombPlot_flux_treat, device="tiff", path=NULL, scale=1, width=14, height=10, dpi=600, limitsize=TRUE, bg="white")

#ggsave("../figures/Ecosphere_pub/GHG_Treatment/Figure3.png", plot=CombPlot_flux_treat, device="png", path=NULL, scale=1, width=14, height=10, dpi=600, limitsize=TRUE, bg="white")
```

```{r Wilcox and Kruskal-Wallis tables}
# Rbind the 3 Wilcox tables:
Wilcox_comb <- rbind(CH4_wilcox.DF, CO2_wilcox.DF, N2O_wilcox.DF)

# Write .csv: 
#write.csv(Wilcox_comb, file = "../figures/pub/tables/Kruskal_Treatment/Wilcox_comb.csv", row.names = T)
...
plot.CH4.flux.Hst <- ggplot(select_fluxes_combined_w_T0, aes(x=history, y=f0_CH4)) +
  ...
  scale_shape_manual(values=c(15, 17, 19), labels = c("Dry", "Interim", "Wet"), name="History")+
  theme_bw() +
  #Remove plot grid lines
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  #Set axis title and text properties, tick marks, and labels
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  coord_cartesian(ylim = c(-16, 55)) +
  stat_pvalue_manual(data = CH4_MultComp.DF_Hst, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = "p.adj", y.position = 45, step.group.by = "date_format", step.increase = 0.02, tip.length = 0.006)
...
```{r Combined GHG Boxplot}
CombPlot_flux_hist <- ggarrange(plot.CH4.flux.Hst, plot.CO2.flux.Hst, plot.N2O.flux.Hst, labels = c("A", "B", "C"), ncol = 1, nrow = 3, legend = "top", common.legend = T, align = "hv")

CombPlot_flux_hist

#ggsave("../figures/Fe_S_manuscript/GHG_flux/Flux_history.tiff", plot=CombPlot_flux_hist, device="tiff", path=NULL, scale=1, width=14, height=10, dpi=600, limitsize=TRUE, bg="white")

#ggsave("../figures/Fe_S_manuscript/GHG_flux/Flux_history.png", plot=CombPlot_flux_hist, device="png", path=NULL, scale=1, width=14, height=10, dpi=600, limitsize=TRUE, bg="white")
```

## GHG flux plots for FeS manuscript
```{r CH4}
...
```{r Combined GHG Boxplot}
CombPlot_flux <- ggarrange(plot.CH4.flux, plot.CO2.flux, plot.N2O.flux, labels = c("A", "B", "C"), ncol = 1, nrow = 3, legend = "top", common.legend = T, align = "hv")
...
```{r GHG Line Plot}
# Rename the data
df <- select_fluxes_combined_w_T0

# Convert 'date_format' to a proper Date object
df <- df %>%
  mutate(date_format = as.Date(date_format, format = "%d-%b"))

# List of gases to plot
gases <- c("f0_CH4", "f0_CO2", "f0_N2O")

# Loop through each gas and create a smoothed plot
for (gas in gases) {
  p <- ggplot(df, aes(x = date_format, y = .data[[gas]])) +
    geom_point(alpha = 0.6) +
    geom_smooth(method = "loess", se = FALSE, color = "blue") +
    labs(
      title = paste("Smoothed Line Plot for", gas, "Over Time"),
      x = "Date",
      y = gas
    ) +
    theme_minimal() +
...
```
```{r list of date breaks for 3 line plots}
date_breaks <- as.Date(c("2025-06-13", "2025-06-28", "2025-07-11", "2025-07-25", "2025-08-11"))
```

```{r CH4 line plot}
# To use open and closed shapes, must create a combined category for History and Plant:
df$history_plant <- as.factor(c(paste(df$history, df$plant, sep = "_")))

# Plotting:
ch4.line.plot <- ggplot(df, x=date_format, y=f0_CH4)  +
  geom_point(aes(x=date_format, y=f0_CH4, color=treatment, shape=history_plant), size = 3, position = position_jitterdodge(jitter.width = 4, dodge.width = 4)) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  scale_shape_manual(labels = c("Dry_No Plant", "Dry_Plant", "Interim_No Plant", "Interim_Plant", "Wet_No Plant", "Wet_Plant"), values = c(0, 15, 2, 17, 1, 19), name="History_Plant")+
  #geom_smooth(aes(x=date_format, y=f0_CH4), formula = y ~ x, method=lm, se=T, color="black")+
  stat_poly_eq(aes(x=date_format, y=f0_CH4, label = paste(after_stat(adj.rr.label), after_stat(p.value.label), sep = "*\", \"*")), data = df, formula = y ~ x, method = "lm", label.y = "top",label.x = "right")+
  theme_bw()  +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          axis.title.y=element_text(margin=margin(r=10)), 
          axis.title.x=element_text(margin=margin(t=10))) +
    theme(axis.title=element_text(vjust=1,size=20),
          axis.text=element_text(size=20), axis.text.x = element_blank(), panel.border =
    ...
    scale_x_date(breaks = date_breaks, date_labels = "%b %d")+
    coord_cartesian(ylim = c(-16, 50))

ch4.line.plot
```

```{r CO2 line plot}
co2.line.plot <- ggplot(df, x=date_format, y=f0_CO2)  +
  geom_point(aes(x=date_format, y=f0_CO2, color=treatment, shape=history_plant), size = 3, position = position_jitterdodge(jitter.width = 4, dodge.width = 4)) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  scale_shape_manual(labels = c("Dry_No Plant", "Dry_Plant", "Interim_No Plant", "Interim_Plant", "Wet_No Plant", "Wet_Plant"), values = c(0, 15, 2, 17, 1, 19), name="History_Plant")+
  #geom_smooth(aes(x=date_format, y=f0_CO2), formula = y ~ x, method=lm, se=T, color="black")+
  stat_poly_eq(aes(x=date_format, y=f0_CO2, label = paste(after_stat(adj.rr.label), after_stat(p.value.label), sep = "*\", \"*")), data = df, formula = y ~ x, method = "lm", label.y = "top",label.x = "right")+
  theme_bw()  +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          axis.title.y=element_text(margin=margin(r=10)), 
          axis.title.x=element_text(margin=margin(t=10))) +
...
```{r N2O line plot}
n2o.line.plot <- ggplot(df, x=date_format, y=f0_N2O)  +
  geom_point(aes(x=date_format, y=f0_N2O, color=treatment, shape=history_plant), size = 3, position = position_jitterdodge(jitter.width = 4, dodge.width = 4)) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  scale_shape_manual(labels = c("Dry_No Plant", "Dry_Plant", "Interim_No Plant", "Interim_Plant", "Wet_No Plant", "Wet_Plant"), values = c(0, 15, 2, 17, 1, 19), name="History_Plant")+
  geom_smooth(aes(x=date_format, y=f0_N2O), formula = y ~ x, method=lm, se=T, color="black")+
  stat_poly_eq(aes(x=date_format, y=f0_N2O, label = paste(after_stat(adj.rr.label), after_stat(p.value.label), sep = "*\", \"*")), data = df, formula = y ~ x, method = "lm", label.y = "top",label.x = "right")+
  theme_bw()  +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          axis.title.y=element_text(margin=margin(r=10)), 
          axis.title.x=element_text(margin=margin(t=10))) +
    theme(axis.title=element_text(vjust=1,size=20),
          ...
          element_rect(colour = "black",size=1)) + 
    theme(axis.ticks.length=unit(0.3,"cm")) + labs(x = "", y = expression(paste("mg N"[2],"O m"^{-2}, " h"^{-1}))) + 
    theme(legend.text=element_text(size=20), legend.title = element_text(size=20), 
          legend.position = "right")+
    theme(strip.text = element_text(size = 20))+
    scale_x_date(breaks = date_breaks, date_labels = "%b %d")
...
```{r Three panel GHG line plot}
GHG_flux_3panel <- ggarrange(ch4.line.plot, co2.line.plot, n2o.line.plot, labels = c("A", "B", "C"), ncol = 1, nrow = 3, legend = "right", common.legend = T, align = "hv")

GHG_flux_3panel

#ggsave("../figures/Fe_S_manuscript/GHG_flux/LinePlot_Flux.tiff", plot=GHG_flux_3panel, device="tiff", path=NULL, scale=1, width=11, height=10, dpi=600, limitsize=TRUE, bg="white")

#ggsave("../figures/Fe_S_manuscript/GHG_flux/LinePlot_Flux.png", plot=GHG_flux_3panel, device="png", path=NULL, scale=1, width=11, height=10, dpi=600, limitsize=TRUE, bg="white")
```
...
ghg.flux.summary.tp <- select_fluxes_combined_w_T0 %>%
  group_by(treatment, plant)%>%
  dplyr::summarise(mean_ch4=mean(f0_CH4), sd_ch4=sd(f0_CH4), mean_co2=mean(f0_CO2), sd_co2=sd(f0_CO2), mean_n2o=mean(f0_N2O), sd_n2o=sd(f0_N2O))

ghg.flux.summary.tp

# format decimal places, 2 decimal places:
ghg.flux.summary.tp$mean_ch4 <- format(round(ghg.flux.summary.tp$mean_ch4, 2), nsmall = 2)
ghg.flux.summary.tp$mean_ch4 <- as.numeric(ghg.flux.summary.tp$mean_ch4)
ghg.flux.summary.tp$sd_ch4 <- format(round(ghg.flux.summary.tp$sd_ch4, 2), nsmall = 2)
ghg.flux.summary.tp$sd_ch4 <- as.numeric(ghg.flux.summary.tp$sd_ch4)

ghg.flux.summary.tp$mean_co2 <- format(round(ghg.flux.summary.tp$mean_co2, 2), nsmall = 2)
ghg.flux.summary.tp$mean_co2 <- as.numeric(ghg.flux.summary.tp$mean_co2)
ghg.flux.summary.tp$sd_co2 <- format(round(ghg.flux.summary.tp$sd_co2, 2), nsmall = 2)
ghg.flux.summary.tp$sd_co2 <- as.numeric(ghg.flux.summary.tp$sd_co2)

ghg.flux.summary.tp$mean_n2o <- format(round(ghg.flux.summary.tp$mean_n2o, 2), nsmall = 2)
ghg.flux.summary.tp$mean_n2o <- as.numeric(ghg.flux.summary.tp$mean_n2o)
ghg.flux.summary.tp$sd_n2o <- format(round(ghg.flux.summary.tp$sd_n2o, 2), nsmall = 2)
ghg.flux.summary.tp$sd_n2o <- as.numeric(ghg.flux.summary.tp$sd_n2o)

# Add plus or minus symbol to divide mean and SD:
ghg.flux.summary.tp$mean_sd_ch4 <- paste(ghg.flux.summary.tp$mean_ch4, "±", ghg.flux.summary.tp$sd_ch4, sep = " ")
ghg.flux.summary.tp$mean_sd_co2 <- paste(ghg.flux.summary.tp$mean_co2, "±", ghg.flux.summary.tp$sd_co2, sep = " ")
ghg.flux.summary.tp$mean_sd_n2o <- paste(ghg.flux.summary.tp$mean_n2o, "±", ghg.flux.summary.tp$sd_n2o, sep = " ")

# Write .csv:
# Warning: running this will overwrite formatting modifications to the file
#write.csv(ghg.flux.summary.tp, file = "../figures/pub/tables/ghg_flux_summary.csv", row.names = T)
```

# PCoA with interim and GHGs on envfit
...
otu.2.rel.dist.pcoa <- cmdscale(otu.2.rel.dist, k=2, eig=TRUE, add=TRUE)
...
# envfit:
fit<-envfit(pcoa_otu_DF, meta.env_GHG, perm=10000, na.rm=T, set.seed(42))
...
vec <- cbind(vec, p) 
vec <-  subset(vec, A$pvals<=0.05) # alpha = 0.05

# Create label column called "parm"
vec$parm<-rownames(vec)
vec$parm<-c("%C", "%N", "Mg", "Ca", "pH", "S", "Mn", "Fe", "Cu", "CEC") # Check that these readable labels match the originals

# Add a column with nudges for nudge_x and nudge_y:
vec$X_nudge <- c(-0.02, -0.02, -0.02, -0.01, -0.01, 0, -0.005, 0, 0.01, -0.02)
vec$Y_nudge <- c(0, 0, 0.005, 0, 0, 0.01, -0.015, 0.02, 0, 0.02)
...
plot16S_PCoA <- ggplot(pcoa.cent.dataframe.trts, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
  ...
  geom_segment(data=vec, aes(x=0,xend=V1,y=0,yend=V2), linewidth=1, arrow = arrow(length = unit(0.2, "cm")),colour="black", alpha = 0.4,inherit.aes=F)+ 
  geom_text(data=vec, aes(x=V1, y=V2), label=vec$parm, nudge_x = vec$X_nudge, nudge_y = vec$Y_nudge, size=4, inherit.aes=F)+
  ggtitle("") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
...
#ggsave("../figures/pub/Figure5.tiff", plot=plot16S_PCoA, device="tiff", path=NULL, scale=1, width=9.5, height=5, dpi=600, limitsize=TRUE)

#ggsave("../figures/pub/16S_PCoA.png", plot=plot16S_PCoA, device="png", path=NULL, scale=1, width=9.5, height=5, dpi=600, limitsize=TRUE)
```
